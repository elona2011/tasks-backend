        ...
        return new Promise((res, rej) => {
            pool.query(`select money from table_user where wx_openid=?`, [obj.wx_openid], function (error, results, fields) {
                ...
                if (results.length) {
                    ...
                    obj.err_code || '', obj.err_code_des || '', obj.partner_trade_no || '', obj.payment_no || '', obj.payment_time || ''], function (error, results, fields) {
                        if (error) rej(error);
                        console.log(results)
        ...
        return new Promise((res, rej) => {
            pool.query(`select money from table_user where wx_openid=?`, [obj.openid], function (error, results, fields) {
                ...
                if (results.length) {
                    ...
                    obj.total_fee || '', obj.transaction_id || '', obj.device_info || '', obj.out_trade_no || '', obj.time_end || ''], function (error, results, fields) {
                        if (error) rej(error);
                        console.log(results)
                        if (results.affectedRows == 1) {
                            if (obj.return_code == 'SUCCESS' && obj.result_code == 'SUCCESS') {
                                pool.query(`update table_user set money=money+?,money_in=money_in+? where wx_openid=?`, [obj.cash_fee, obj.cash_fee, obj.openid], function (error, results, fields) {
                                    if (error) {
                                        console.log(error)
                                        return rej(error);
                                    }
